    ...
    <section class="mb-8">
        ...
        <div class="grid grid-cols-1 gap-3">
            <?php foreach($assigned_members as $m): ?>
                ...
                ?>
                <div class="flex items-center justify-between bg-white p-3 rounded-xl shadow-sm border border-slate-100 hover:border-blue-100 transition-all">
                    <div class="flex items-center gap-4">
                    <?php 
                    $foto_path = 'uploads/profile_pics/' . ($m['profile_photo'] ?? '');
                    if (!empty($m['profile_photo']) && file_exists($foto_path)): ?>
                        <img src="<?php echo $foto_path; ?>" class="w-10 h-10 rounded-lg object-cover shadow-sm">
                    <?php else: ?>
                        <div class="w-10 h-10 bg-slate-900 rounded-lg flex items-center justify-center text-white text-xs font-black italic">
                            <?php echo strtoupper(substr($m['full_name'], 0, 1)); ?>
                        </div>
                    <?php endif; ?>

                    <div>
                        <p class="text-xs font-black text-slate-800 leading-tight uppercase italic tracking-tight">
                    ...
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center border <?php echo $s_color; ?> font-black text-xs" title="<?php echo ucfirst($status); ?>">
                        <?php echo $s_icon; ?>
                    </div>
                </div>
            <?php endforeach; ?>
    ...
    <section>
        ...
        <div class="space-y-4">
            <?php foreach($songs as $s): ?>
                <div class="bg-white border border-slate-100 p-4 rounded-2xl shadow-md shadow-slate-200/50 group hover:scale-[1.02] transition-transform">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h3 class="text-lg font-black text-slate-800 leading-tight mb-1 uppercase italic tracking-tighter">
                                <?php echo htmlspecialchars($s['title']); ?>
                            </h3>
                            <div class="flex items-center gap-2">
                                <span class="bg-blue-50 text-blue-600 font-black text-[9px] tracking-widest px-2 py-0.5 rounded-md">
                                    Tono: <?php echo $s['musical_key']; ?>
                                </span>
                            </div>
                        </div>
                        <div class="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-lg shadow-inner group-hover:bg-blue-50 transition-colors">
                            🎵
                        </div>
                    </div>
                    ...
                    <?php if(!empty($s['youtube_link'])): ?>
                        <a href="<?php echo $s['youtube_link']; ?>" target="_blank" 
                           class="flex items-center justify-center gap-2 w-full bg-[#FF0000] hover:bg-[#CC0000] text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 shadow-md shadow-red-100">
                            <span class="text-lg">▶</span> Escuchar en YouTube
                        </a>
                    <?php else: ?>
                        <div class="text-center p-3 bg-slate-50 rounded-xl text-slate-300 text-[9px] font-black uppercase tracking-widest border border-dashed border-slate-200">
                            Link no disponible
                        </div>
                    <?php endif; ?>
                </div>
