...
// Validar que tenemos un ID de evento
...
if (isset($_POST['save_setlist'])) {
    try {
        // 1. Limpiar setlist anterior para evitar duplicados si se edita
        $stmt_del = $pdo->prepare("DELETE FROM event_songs WHERE event_id = ?");
        $stmt_del->execute([$event_id]);
        
        // 2. Insertar nuevas canciones si hay seleccionadas
...
<div class="container mx-auto px-4 max-w-2xl py-12">
    <div class="mb-10 text-center">
        <h1 class="text-4xl font-black text-slate-900 tracking-tighter uppercase">Planificar Setlist</h1>
        <p class="text-blue-600 font-bold text-sm uppercase tracking-widest mt-2">
            <?php echo htmlspecialchars($info_evento['description'] ?? 'Evento'); ?>
        </p>
    </div>

    <div class="relative mb-8">
        <select id="songPicker" class="w-full p-5 bg-white border border-slate-200 rounded-[2rem] shadow-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 appearance-none">
    ...
    <form method="POST">
        ...
        <button type="submit" name="save_setlist" class="w-full bg-slate-900 text-white py-6 rounded-[2.5rem] font-black text-xs uppercase tracking-[0.2em] shadow-2xl hover:bg-blue-600 transition-all">
            Confirmar Plan de Alabanza
        </button>
    </form>
</div>
...
songPicker.addEventListener('change', function() {
    ...
    row.className = "flex items-center justify-between p-5 bg-white rounded-[2rem] border border-slate-100 shadow-sm";
    row.innerHTML = `
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-[9px] text-slate-400 font-bold uppercase">${artist}</p>
            </div>
        </div>
        <div class="flex items-center gap-4">
            <span class="text-blue-600 font-black text-[10px]">${key}</span>
            <button type="button" onclick="this.parentElement.parentElement.remove()" class="text-slate-300 hover:text-red-500 font-bold px-2">✕</button>
        </div>
    `;
